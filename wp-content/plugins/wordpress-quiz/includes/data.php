...
function validate_questions(mixed $input): array|\WP_Error
{
    ...
    foreach ($input as $question) {
        ...
        if (
            !is_string($id) ||
            !preg_match('/^[a-zA-Z0-9_-]{1,64}$/D', $id) ||
            isset($ids[$id]) ||
            !is_string($title) ||
            !trim($title) ||
            mb_strlen($title) > 250 ||
            ...
            return new \WP_Error(
                'schema',
                'У каждого вопроса нужны уникальный ID, текст до 250 символов и от 2 до 20 вариантов.',
            );
        }
        $ids[$id] = true;
        $clean_options = [];
        ...
        foreach ($options as $option) {
            ...
            if (
                !is_string($oid) ||
                !preg_match('/^[a-zA-Z0-9_-]{1,64}$/D', $oid) ||
                isset($option_ids[$oid]) ||
                !is_string($label) ||
                !trim($label) ||
                mb_strlen($label) > 200
            ) {
                return new \WP_Error(
                    'schema',
                    'У каждого варианта нужны уникальный ID и текст до 200 символов.',
                );
            }
            $option_ids[$oid] = true;
...
function submit_lead(\WP_REST_Request $request): \WP_REST_Response|\WP_Error
{
    ...
    if (!is_array($data)) {
        return api_error('Ожидается JSON.');
    }
    foreach (['requestId', 'revision', 'name', 'email', 'phone', 'website'] as $key) {
        if (isset($data[$key]) && !is_string($data[$key])) {
            return api_error('Некорректные поля.');
        }
    }
    if (!empty($data['website'])) {
        return api_error('Не удалось отправить форму.');
    }
    ...
    if (!isset($data['quizId']) || !is_int($data['quizId'])) {
        return api_error('Не указан квиз.');
    }
    $id = $data['quizId'];
    $post = get_post($id);
    ...
    if (is_wp_error($questions)) {
        return api_error('Квиз пока не настроен.', 409);
    }
    ...
    if (true !== ($data['consent'] ?? false)) {
        return api_error('Необходимо согласие на обработку данных.');
    }
    $answers = $data['answers'] ?? null;
    if (!is_array($answers) || count($answers) !== count($questions)) {
        return api_error('Ответьте на все вопросы.');
    }
    $snapshot = [];
    foreach ($questions as $question) {
        ...
        if (!$matched) {
            return api_error('Выбран недопустимый вариант ответа.');
        }
        $snapshot[] = [
    ...
    $payload = [
        ...
        'quizId' => $id,
        'quizTitle' => $post->post_title,
        'revision' => $data['revision'],
        'contact' => ['name' => $name, 'email' => sanitize_email($email), 'phone' => $phone],
        'answers' => $snapshot,
    ...
    if ($existing) {
        return hash_equals($existing->payload_hash, $hash)
            ? receipt($existing)
            : api_error('Эта отправка уже использована. Обновите страницу.', 409);
    }
    $rate_key = 'wpq_rate_' . hash_hmac('sha256', $_SERVER['REMOTE_ADDR'] ?? 'unknown', wp_salt());
    $count = (int) get_transient($rate_key);
    if ($count >= 10) {
        return api_error('Слишком много отправок. Попробуйте через 10 минут.', 429);
    }
    ...
    deliver($lead_id);
    return receipt(
        $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE id = %d", $lead_id)),
        201,
    );
}
...
function deliver(int $id): bool
{
    ...
    $locked = $wpdb->query(
        $wpdb->prepare(
            "UPDATE $table SET status = 'sending', attempts = attempts + 1, updated_at = %s WHERE id = %d AND (status IN ('pending','failed') OR (status = 'sending' AND updated_at < %s))",
            $now,
            $id,
            $stale,
        ),
    ...
    if (!$url || !$token) {
        ...
        $response = wp_remote_post($url, [
            ...
            'headers' => [
                ...
                'Idempotency-Key' => $lead->request_id,
            ],
            'body' => $lead->payload,
        ]);
        if (is_wp_error($response)) {
            ...
            if (
                ...
                $error = 'CRM не подтвердила получение (HTTP ' . $code . ').';
            }
        }
    }
    $wpdb->update(
