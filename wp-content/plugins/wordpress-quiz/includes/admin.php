...
function editor(\WP_Post $post): void
{
    ...
    echo '<div id="wpq-editor"></div><textarea hidden id="wpq-schema" name="wpq_schema">' .
        esc_textarea(wp_json_encode(is_array($questions) ? $questions : [])) .
        '</textarea>';
    echo '<p>Вставка на страницу: блок <strong>«Квиз»</strong> или <code>[wordpress_quiz id="' .
        (int) $post->ID .
        '"]</code>.</p>';
}
...
add_action(
    ...
    static function (int $id) {
        if (
            ...
            !isset($_POST['wpq_nonce'], $_POST['wpq_schema']) ||
            !is_string($_POST['wpq_nonce']) ||
            !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['wpq_nonce'])), 'wpq_save')
        ) {
            return;
        }
        ...
        if (is_wp_error($questions)) {
            set_transient(
                'wpq_notice_' . get_current_user_id(),
...
add_action('admin_notices', static function () {
    ...
    if ($notice) {
        ...
        echo '<div class="notice notice-error"><p>Вопросы не сохранены: ' .
            esc_html($notice) .
            ' Предыдущая версия сохранена.</p></div>';
    }
});
...
function leads_page(): void
{
    ...
    $rows = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT * FROM $table ORDER BY id DESC LIMIT 20 OFFSET %d",
            ($page - 1) * 20,
        ),
    );
    ...
    if (isset($_GET['retried'])) {
        echo '<div class="notice notice-info"><p>Попытка доставки выполнена. Проверьте статус заявки.</p></div>';
    }
    $configured =
        (defined('WPQ_CRM_URL') || getenv('WPQ_CRM_URL')) &&
        (defined('WPQ_CRM_TOKEN') || getenv('WPQ_CRM_TOKEN'));
    echo '<p><strong>CRM:</strong> ' .
        ($configured
            ? 'подключение настроено на сервере'
            : 'не настроена — заявки будут сохранены с ошибкой доставки') .
        '</p>';
    ...
    foreach ($rows as $row) {
        ...
        echo '<tr><td>' .
            ...
            '<br><strong>' .
            esc_html($payload['quizTitle']) .
            '</strong><br><small>' .
            esc_html($row->request_id) .
            '</small></td><td>';
        foreach ($payload['contact'] as $value) {
            echo esc_html($value) . '<br>';
        }
        echo '<small>Согласие: ' .
            esc_html($payload['consent']['version']) .
            '</small></td><td><ol>';
        foreach ($payload['answers'] as $answer) {
            echo '<li><strong>' .
                esc_html($answer['question']) .
                '</strong><br>' .
                esc_html($answer['answer']) .
                '</li>';
        }
        echo '</ol></td><td><strong>' .
            esc_html($labels[$row->status] ?? $row->status) .
            '</strong><br>Попыток: ' .
            (int) $row->attempts;
        if ($row->crm_id) {
            echo '<br>ID: ' . esc_html($row->crm_id);
        }
        if ($row->last_error) {
            echo '<br>' . esc_html($row->last_error);
        }
        if ('delivered' !== $row->status) {
            echo '<form method="post" action="' .
                esc_url(admin_url('admin-post.php')) .
                '"><input type="hidden" name="action" value="wpq_retry"><input type="hidden" name="lead_id" value="' .
                (int) $row->id .
                '">';
    ...
    if (!$rows) {
        echo '<tr><td colspan="4">Заявок пока нет. Пройдите квиз на демонстрационной странице.</td></tr>';
    }
    echo '</tbody></table><p>' .
        wp_kses_post(
            paginate_links([
