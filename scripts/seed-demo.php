...
if (
    !defined('WP_CLI') ||
    !WP_CLI ||
    ('local' !== wp_get_environment_type() && '1' !== getenv('WPQ_ALLOW_DEMO_SETUP'))
) {
    exit(1);
}
    ...
    'marketing' => [
        ...
        [
            [
                ...
                [
                    'leads' => 'Получать больше заявок',
                    'sales' => 'Увеличить продажи',
                    'start' => 'Запустить новый проект',
                ],
            ],
            [
                'channel',
                'Что уже используете для продвижения?',
                ['ads' => 'Контекстную рекламу', 'seo' => 'SEO и контент', 'none' => 'Пока ничего'],
            ],
            [
                'budget',
                'Какой бюджет планируете в месяц?',
                [
                    'small' => 'До 100 000 ₽',
                    'medium' => '100 000–300 000 ₽',
                    'large' => 'Более 300 000 ₽',
                    'help' => 'Нужна помощь с оценкой',
                ],
            ],
        ],
    ],
    'website' => [
        ...
        [
            ...
            [
                ...
                [
                    'soon' => 'В ближайший месяц',
                    'later' => 'Через 2–3 месяца',
                    'explore' => 'Пока изучаю варианты',
                ],
            ],
        ],
    ],
...
foreach ($definitions as $slug => [$title, $rows]) {
    ...
    if (!$id || !get_post($id)) {
        $id = wp_insert_post(
            ['post_type' => 'wpq_quiz', 'post_title' => $title, 'post_status' => 'publish'],
            true,
        );
        if (is_wp_error($id)) {
